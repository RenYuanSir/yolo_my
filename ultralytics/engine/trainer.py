...
class BaseTrainer:
        ...
        while True:
            ...
            for i, batch in pbar:
                ...
                if RANK in {-1, 0}:
                    ...
                    # 统一使用格式字符串
                    # 构建固定的描述字符串格式：epochs + GPU_mem + losses + batch_size + imgsz
                    desc_format = "%11s" * 2  # epochs和GPU_mem
                    desc_format += "%11.4g" * loss_length  # 损失值（浮点数）
                    desc_format += "%11.4g" * 2  # batch_size和imgsz（浮点数）
                    ...
                    pbar.set_description(desc_format % tuple(desc_args))
    ...
    def optimizer_step(self):
        ...
        try:
            ...
            for param_group in self.optimizer.param_groups:
                for param in param_group['params']:
                    if param.grad is not None and not torch.isfinite(param.grad).all():
                        LOGGER.warning(f"检测到Inf/NaN梯度，将设置为0")
            ...
            if "No inf checks were recorded for this optimizer" in str(e):
                LOGGER.warning(f"GradScaler错误: {e}")
                # 作为替代方案，直接进行优化步骤
                LOGGER.warning("使用普通优化器步骤作为替代方案")
                self.optimizer.step()  # 普通优化器步骤
            else:
    ...
    def progress_string(self):
        ...
        # 默认的列标题
        loss_names = self.label_loss_items(None)
        loss_names_count = len(loss_names)
