...
class Instances:
    ...
    def __init__(
        ...
        if cluster_ids is not None:
            if isinstance(cluster_ids, torch.Tensor):
                self.cluster_ids = cluster_ids.numpy() if cluster_ids.numel() > 0 else None
            elif isinstance(cluster_ids, np.ndarray) and cluster_ids.size > 0:
                self.cluster_ids = cluster_ids
            elif isinstance(cluster_ids, list) and len(cluster_ids) > 0:
                self.cluster_ids = np.array(cluster_ids)
            else:
                self.cluster_ids = None
                LOGGER.debug(f"Instances: 提供的cluster_ids无效或为空: {type(cluster_ids)}")
        else:
            self.cluster_ids = None

        if h_rel is not None:
            if isinstance(h_rel, torch.Tensor):
                self.h_rel = h_rel.numpy() if h_rel.numel() > 0 else None
            elif isinstance(h_rel, np.ndarray) and h_rel.size > 0:
                self.h_rel = h_rel
            elif isinstance(h_rel, list) and len(h_rel) > 0:
                self.h_rel = np.array(h_rel)
            else:
                self.h_rel = None
                LOGGER.debug(f"Instances: 提供的h_rel无效或为空: {type(h_rel)}")
        else:
            self.h_rel = None
    ...
    def bbox_format(self):
        """Return the current bounding box format."""
        return self._bboxes.format
    ...
    def remove_zero_area_boxes(self):
        ...
        Returns:
            bool: 如果所有边界框都有效（面积大于0），则返回True，否则返回False
        """
        if len(self) == 0:
            return False
        
        # 计算所有边界框的面积
        areas = self.bbox_areas()
        
        # 找出面积大于0的边界框
        keep = areas > 0
        if not all(keep):
            self._bboxes = self._bboxes[keep]
            if len(self.segments):
        ...
        if self.cluster_ids is not None:
            self.cluster_ids = self.cluster_ids[keep]
            
        if self.h_rel is not None:
            self.h_rel = self.h_rel[keep]
    ...
    def concatenate(cls, instances_list: List["Instances"], axis=0) -> "Instances":
        ...
        return Instances(bboxes.bboxes, segments, keypoints, bbox_format=instances_list[0].bbox_format, 
