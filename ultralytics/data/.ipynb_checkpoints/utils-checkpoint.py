        ...
        if os.path.isfile(lb_file):
            ...
            if len(lb) > 0:
                ...
                if use_keypoints:
                    ...
                    if lb.shape[1] != 6:
                        LOGGER.warning(f"{prefix}WARNING ⚠️ Labels with cluster_id should have 6 columns, found {lb.shape[1]}.")
                        nc = 1
                elif not (use_segments or use_obb):
                    if lb.shape[1] != 5:
                        LOGGER.warning(f"{prefix}WARNING ⚠️ Standard labels should have 5 columns, found {lb.shape[1]}.")
                        nc = 1

                # Box properties checks
                assert (lb[:, 1:5] <= 1).all(), f"non-normalized or out of bounds coordinates {lb[:, 1:][lb[:, 1:] > 1]}"
                if has_h_rel:          # 7 列标签，允许最后一列取 -1
                    assert (lb[:, :6] >= 0).all(), \
                        f"negative values in bbox/cluster_id columns {lb[:, :6][lb[:, :6] < 0]}"
                elif has_cluster_id:   # 6 列标签
                    assert (lb[:, :5] >= 0).all(), \
                        f"negative values in bbox columns {lb[:, :5][lb[:, :5] < 0]}"
                else:
