...
class YOLODataset(BaseDataset):
        ...
        try:
            with ThreadPool(NUM_THREADS) as pool:
                ...
                for im_file, lb, shape, segs, kpts, nm_f, ne_f, nc_f, ms_f in pbar:
                    ...
                    # 确保所有标签字段存在
                    label_dict.setdefault("cluster_id", torch.zeros((0, 1)))
                    label_dict.setdefault("h_rel", torch.zeros((0, 1)))
    ...
    def update_labels_info(self, label):
        ...
        if "cluster_ids" in label:
            label["cluster_ids"] = label["cluster_ids"]
        elif "cluster_id" in label:
            label["cluster_ids"] = label["cluster_id"]
        
        if "h_rel" in label:
            label["h_rel"] = label["h_rel"]
    ...
    def collate_fn(batch):
        ...
        for i, samp in enumerate(batch):
            ...
            # 额外列保持行数一致
            cid = torch.as_tensor(samp.get('cluster_id', np.zeros((n, 1))), dtype=torch.float32)
            hrel = torch.as_tensor(samp.get('h_rel', np.zeros((n, 1))), dtype=torch.float32)
    
            # 确保 cluster_id 和 h_rel 的维度与 bboxes 一致
            if cid.shape[0] != n:
                print(f"Warning: cluster_id has inconsistent dimension for sample {i}: {cid.shape[0]} != {n}")
            ...
            # 打印当前样本的维度信息，便于调试
            print(f"Sample {i}: cls={cls.shape}, boxes={boxes.shape}, cluster_id={cid.shape}, h_rel={hrel.shape}")
    
            # 检查当前数据样本格式
            assert cls.shape[0] == boxes.shape[0] == cid.shape[0] == hrel.shape[0], \
                f"样本标签维度不一致！ cls: {cls.shape}, bboxes: {boxes.shape}, cluster_id: {cid.shape}, h_rel: {hrel.shape}"
