...
class Format:
    ...
    def __call__(self, labels):
        ...
        """
        # 保存原始标签的副本，确保我们不会丢失任何信息
        labels_copy = labels.copy()
        img = labels_copy.pop("img")
        h, w = img.shape[:2]
        
        # 保存所有非标准字段，以便后续处理
        extra_fields = {}
        for k in list(labels_copy.keys()):
            if k not in ["img", "instances", "cls"]:
                extra_fields[k] = labels_copy.pop(k)
        
        # 检查instances是否存在，如果不存在则创建一个空的实例
        if "instances" not in labels_copy:
            # 导入必要的类
            from ultralytics.utils.instance import Instances
            ...
            # 检查cls是否存在
            if "cls" in labels_copy:
                cls = labels_copy.pop("cls")
            else:
                cls = np.zeros(0)
                
            # 尝试获取实例数量信息
            nl = 0
            if isinstance(cls, (np.ndarray, torch.Tensor)) and len(cls) > 0:
                nl = len(cls)
            elif "bboxes" in extra_fields and isinstance(extra_fields["bboxes"], (np.ndarray, torch.Tensor)):
                nl = len(extra_fields["bboxes"])
        else:
            instances = labels_copy.pop("instances")
        ...
        if self.return_mask:
            ...
            labels_copy["masks"] = masks
            
        # 添加处理后的字段到结果中    
        result = {}
        result["img"] = self._format_img(img)
        # 确保cls是numpy数组再转tensor
        if isinstance(cls, torch.Tensor):
            result["cls"] = cls
        else:
            result["cls"] = torch.from_numpy(cls) if nl else torch.zeros(nl, dtype=torch.int64)
        result["bboxes"] = torch.from_numpy(instances.bboxes) if nl else torch.zeros((nl, 4))
        
        if self.return_keypoint:
            result["keypoints"] = torch.from_numpy(instances.keypoints) if nl and hasattr(instances, 'keypoints') and instances.keypoints is not None else torch.zeros((nl, 0, 3))
        ...
        # 保持与原始实现的兼容性，复制labels_copy中可能存在的其他字段    
        for k, v in labels_copy.items():
            if k not in result:
                result[k] = v
    ...
    def __call__(self, labels):
        ...
        Args:
            labels (dict): 包含以下键的字典：
                - 'img': 图像张量或numpy数组
                - 'cls': 类别标签
                - 'instances': 包含'bboxes'、'segments'和'keypoints'的对象
