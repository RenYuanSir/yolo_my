...
def build_yolo_dataset(args, img_path, batch, data, mode="train", rect=False, stride=32):
    """
    Build a YOLO dataset.

    Args:
        args (dict): Build arguments.
        img_path (str): Image path.
        batch (int): Batch size.
        data (str | dict): Path to data.yaml or data dictionary.
        mode (str): 'train' or 'val'.
        rect (bool): Rectangular images.
        stride (int): Image stride.

    Returns:
        (Dataset): YOLO dataset.
    """
    # Load data dictionary if provided a path
    if isinstance(data, str):
        try:
            data = check_det_dataset(data)
            LOGGER.info(f"成功加载数据集配置: {data}")
        except Exception as e:
            LOGGER.error(f"加载数据集配置失败: {e}")
            # 提供默认配置以防出错
            data = {"path": str(Path(data).parent), "nc": 6, "names": ["class0", "class1", "class2", "class3", "class4", "class5"]}

    # Further checks for 'path' key in data dictionary
    if not isinstance(data, dict):
        LOGGER.error(f"数据不是字典类型: {type(data)}. 创建默认字典.")
        data = {"path": str(Path(img_path).parent), "nc": 6, "names": ["class0", "class1", "class2", "class3", "class4", "class5"]}
    elif "path" not in data:
        LOGGER.error("数据字典中没有'path'键. 添加默认路径.")
        data["path"] = str(Path(img_path).parent)
    
    # 确保数据字典包含必要的键
    if "nc" not in data:
        LOGGER.warning("数据字典中没有'nc'键. 使用默认值6.")
        data["nc"] = 6
    
    if "names" not in data:
        LOGGER.warning("数据字典中没有'names'键. 使用默认类名.")
        data["names"] = [f"class{i}" for i in range(data["nc"])]
    
    # 对于番茄数据集，确保有这些键
    if "has_cluster_id" not in data:
        data["has_cluster_id"] = False
    
    if "has_h_rel" not in data:
        data["has_h_rel"] = False
    ...
    try:
        ...
        if task == "tomato":
            ...
            dataset = YOLODataset(**dataset_args)
