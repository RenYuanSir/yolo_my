...
class BaseModel(nn.Module):
    ...
    def profile(self, inputs=(640, 640), repeats=10, P6=False):
        """
        Profile model by running inference several times.

        Args:
            inputs (list | tuple): Input dimensions.
            repeats (int): Number of repeats.
            P6 (bool): Whether to use P6 model.
        """
        import torch

        if isinstance(inputs, (list, tuple)):
            if len(inputs) == 2:
                inputs = torch.zeros((1, 3, inputs[0], inputs[1]), device=next(self.parameters()).device)
            elif len(inputs) == 3:
                inputs = torch.zeros((1, inputs[0], inputs[1], inputs[2]), device=next(self.parameters()).device)
            else:
                inputs = torch.zeros(inputs, device=next(self.parameters()).device)
        # Warmup
        for _ in range(2):
            self._predict_once(inputs)
        # Profile
        self._predict_once(inputs, profile=True)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect_Efficient_Tomato):
            LOGGER.info("检测到番茄检测模型头部，设置task为'tomato'")
            self.task = "tomato"
...
class TomatoDetectionModel(DetectionModel):
    ...
    def __init__(self, cfg="yolov12-tomato.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, Detect_Efficient_Tomato):  
            ...
            def _get_feature_maps(tensor_input):
                output_dict = self.forward(tensor_input)
                return output_dict['features']  # forward
            # Create dummy input and get feature maps
            dummy_input = torch.zeros(1, ch, s, s)
            feature_maps = _get_feature_maps(dummy_input)

            # Calculate stride based on the feature maps
            # This is now correct as it iterates over the list of tensors
            m.stride = torch.tensor([s / x.shape[-2] for x in feature_maps])

            # Assign the calculated stride back to the main model if needed
            self.stride = m.stride

            # Initialize biases now that stride is known
            m.bias_init()
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                n = 1
            if m in {C3k2, C3k2_MambaOut}:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            if m in {A2C2f, A2C2f_DYT}: 
                legacy = False
        ...
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, Detect_Efficient_Tomato, Detect_Efficient}:
            args.append([ch[x] for x in f])
            if m is Segment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, Segment, Pose, OBB, Detect_Efficient_Tomato, Detect_Efficient}:
                m.legacy = legacy
