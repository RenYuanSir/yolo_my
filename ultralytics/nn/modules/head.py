...
__all__ = "Detect", "Segment", "Pose", "Classify", "OBB", "RTDETRDecoder", "v10Detect","Detect_Efficient_Tomato","Detect_Efficient"
...
class Detect_Efficient_Tomato(Detect_Efficient):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            print(f"xi.shape = {x[i].shape}") 
        
        if self.training:
            # 训练阶段返回字典格式，确保与loss.py中TomatoDetectWithRankLoss.__call__方法兼容
            if self.training and (self.anchors.numel() == 0 or self.strides.numel() == 0):
                self.anchors, self.strides = (x.transpose(0, 1) for x in make_anchors(bbox_cls_features, self.stride, 0.5))

            return {"features": bbox_cls_features, "h_pos": h_pos_features}
        
        # 推理时初始化anchors和strides
        if self.dynamic or self.shape != shape:
            self.anchors, self.strides = (x.transpose(0, 1) for x in make_anchors(bbox_cls_features, self.stride, 0.5))
            self.shape = shape
        ...
        return y if self.export else (y, {"features": bbox_cls_features, "h_pos": h_pos_features})
