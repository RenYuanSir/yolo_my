...
def test_data_loading():
    """测试数据加载功能"""
    # 获取数据路径 - 修复路径
    root_path = Path('D:/TomatoDataset')
    data_path = root_path / "roboflow-v2/data.yaml"
    ...
    print(f"数据集属性: has_cluster_ids={dataset.has_cluster_ids}, has_h_rel={dataset.has_h_rel}")
    ...
    # 检查番茄特有字段
    has_cluster_ids = any('cluster_ids' in label or 'cluster_id' in label for label in labels)
    has_h_rel = any('h_rel' in label for label in labels)
    
    print(f"cluster_ids字段存在: {has_cluster_ids}")
